#!/usr/bin/env bash
#
# Reserve the @xberg-io/* npm package names with v0.0.1 placeholders.
#
# Why: npm trusted publishing (OIDC) and the scoped @xberg-io org require each
...
#   - @xberg-io/xberg, @xberg-io/xberg-wasm   (top-level entry points)
#   - @xberg-io/xberg-cli                     (CLI proxy)
#   - 8 napi platform packages                (optionalDependencies of @xberg-io/xberg)
#
# Usage:
#   scripts/publish/placeholders/npm-placeholders.sh [--dry-run]
#
# Requirements:
#   - npm on PATH, logged in to the @xberg-io org (`npm whoami` must succeed)
#
set -euo pipefail
...
PLATFORMS=(
  "@xberg-io/xberg-linux-x64-gnu|linux|x64|glibc"
  "@xberg-io/xberg-linux-arm64-gnu|linux|arm64|glibc"
  "@xberg-io/xberg-linux-x64-musl|linux|x64|musl"
  "@xberg-io/xberg-linux-arm64-musl|linux|arm64|musl"
  "@xberg-io/xberg-darwin-x64|darwin|x64|"
  "@xberg-io/xberg-darwin-arm64|darwin|arm64|"
  "@xberg-io/xberg-win32-x64-msvc|win32|x64|"
  "@xberg-io/xberg-win32-arm64-msvc|win32|arm64|"
)
...
main() {
  # Platform packages first so @xberg-io/xberg's optionalDependencies resolve.
