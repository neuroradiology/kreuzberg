...
interface TooltipPayload {
	...
	color?: string;
	fill?: string;
	payload?: { name: string; fullName?: string };
}
...
const ThroughputTooltip = ({ active, payload }: TooltipProps) => {
	...
	const fullName = payload[0].payload?.fullName || "Unknown";

	return (
		<div className="rounded-lg border border-border bg-card p-3 shadow-lg" data-testid="throughput-tooltip-content">
 ...
 * ```
 */
export function ThroughputChart({ framework, fileType, ocrMode }: ThroughputChartProps) {
			...
			<Card data-testid="chart-throughput-error">
				...
				<CardContent>
					<Alert variant="destructive" data-testid="error-throughput-chart">
						<AlertDescription>Error loading throughput data: {error.message}</AlertDescription>
					</Alert>
				</CardContent>
			</Card>
		);
	...
	return (
		<Card data-testid="chart-throughput">
			<CardHeader>
				...
				<p className="mt-1 text-sm font-medium text-foreground">Higher is better</p>
			</CardHeader>
			<CardContent>
				<ResponsiveContainer width="100%" height={400}>
					...
					>
						...
						<Legend wrapperStyle={{ paddingTop: "20px" }} className="text-sm text-muted-foreground" />
				...
				<div className="mt-6 grid grid-cols-3 gap-4 rounded-lg bg-muted/50 p-4">
					<div className="space-y-1">
						<div className="flex items-center gap-2">
							<div className="h-3 w-3 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p50 }} />
							<span className="text-sm font-medium">P50 (Median)</span>
						</div>
						<p className="text-xs text-muted-foreground">Typical throughput for 50% of requests</p>
					</div>
					<div className="space-y-1">
						<div className="flex items-center gap-2">
							<div className="h-3 w-3 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p95 }} />
							<span className="text-sm font-medium">P95 (95th %ile)</span>
						</div>
						<p className="text-xs text-muted-foreground">Throughput for 95% of requests</p>
					</div>
					<div className="space-y-1">
						<div className="flex items-center gap-2">
							<div className="h-3 w-3 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p99 }} />
							<span className="text-sm font-medium">P99 (99th %ile)</span>
						</div>
						<p className="text-xs text-muted-foreground">Throughput for slowest 1% of requests</p>
					</div>
				</div>
			</CardContent>
		</Card>
