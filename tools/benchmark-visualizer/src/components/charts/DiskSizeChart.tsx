	...
	if (active && payload && payload.length) {
		...
		return (
			<div className="bg-slate-900 text-white p-3 rounded-lg border border-slate-700 shadow-lg">
				...
				{data?.description && <p className="text-xs text-slate-300 mt-1">{data.description}</p>}
...
export function DiskSizeChart() {
	...
	return (
		<Card data-testid="chart-disk-size">
			<CardHeader>
				<CardTitle>Disk Size Comparison</CardTitle>
				<p className="text-sm text-muted-foreground mt-2">Framework disk sizes sorted from smallest to largest.</p>
			...
			<CardContent>
				<ResponsiveContainer width="100%" height={400}>
					...
					>
						...
						<YAxis label={{ value: "Size (MB)", angle: -90, position: "insideLeft" }} tick={{ fontSize: 12 }} />
						<Tooltip
