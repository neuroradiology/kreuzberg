	...
	return (
		<div className="bg-slate-900 border border-slate-700 rounded-md p-3 shadow-lg">
			...
			{payload.map((entry: TooltipPayload) => (
				<p key={entry.name} style={{ color: entry.fill || "#ffffff" }} className="text-sm">
...
export function MemoryChart({ framework, fileType, ocrMode }: MemoryChartProps) {
	...
	return (
		<Card data-testid="chart-memory">
			<CardHeader>
				...
				<p className="mt-1 text-sm font-medium text-foreground">Lower is better</p>
			</CardHeader>
			<CardContent>
				<ResponsiveContainer width="100%" height={400} data-testid="memory-responsive-container">
					<BarChart data={chartData} margin={{ top: 20, right: 30, left: 0, bottom: 60 }} data-testid="memory-barchart">
						...
						/>
						<YAxis label={{ value: "Memory (MB)", angle: -90, position: "insideLeft" }} data-testid="memory-yaxis" />
						<Tooltip
							content={<CustomTooltip />}
							data-testid="memory-tooltip"
							cursor={{ fill: "rgba(0, 0, 0, 0.05)" }}
						/>
						<Legend wrapperStyle={{ paddingTop: "20px" }} data-testid="memory-legend" verticalAlign="top" height={36} />
						<Bar
							dataKey="p50"
				...
				<div className="mt-6 grid grid-cols-3 gap-4 text-sm">
					<div className="flex items-center gap-2">
						<div className="w-4 h-4 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p50 }} />
						<span className="text-muted-foreground">p50: Typical memory usage</span>
					</div>
					<div className="flex items-center gap-2">
						<div className="w-4 h-4 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p95 }} />
						<span className="text-muted-foreground">p95: High memory usage</span>
					</div>
					<div className="flex items-center gap-2">
						<div className="w-4 h-4 rounded" style={{ backgroundColor: PERCENTILE_COLORS.p99 }} />
