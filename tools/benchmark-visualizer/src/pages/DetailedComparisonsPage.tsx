...
export function DetailedComparisonsPage() {
	...
	const tableData = useMemo(() => {
		...
		const sorted = rows.sort((a, b) => {
			const aVal = a[sortField as keyof BenchmarkTableRow];
			const bVal = b[sortField as keyof BenchmarkTableRow];
	...
	return (
		<div data-testid="page-comparisons" className="container mx-auto p-4">
			...
			<div className="mb-4 flex items-center justify-between">
				...
				{totalRows === 0 && (
					<div className="text-sm text-muted-foreground">No data available for the selected filters</div>
				)}
			</div>
						...
						<TableHeader className="bg-background dark:bg-background sticky top-0 z-10 border-b-2 border-border">
							<TableRow>
								...
								{(metricView === "throughput" || metricView === "all") && (
									<>
										<TableHead
											data-testid="table-header-throughput-p50"
											className="min-w-32 text-right font-semibold text-foreground"
										>
											<SortButton field="throughputP50" label="Throughput p50 (MB/s)" />
										</TableHead>
										<TableHead
											data-testid="table-header-throughput-p95"
											className="min-w-32 text-right font-semibold text-foreground"
										>
											<SortButton field="throughputP95" label="Throughput p95 (MB/s)" />
										</TableHead>
										<TableHead
											data-testid="table-header-throughput-p99"
											className="min-w-32 text-right font-semibold text-foreground"
										>
								...
								{(metricView === "duration" || metricView === "all") && (
									<>
										<TableHead
											data-testid="table-header-duration-p50"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="durationP50" label="Duration p50 (ms)" />
										</TableHead>
										<TableHead
											data-testid="table-header-duration-p95"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="durationP95" label="Duration p95 (ms)" />
										</TableHead>
										<TableHead
											data-testid="table-header-duration-p99"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="durationP99" label="Duration p99 (ms)" />
										</TableHead>
										<TableHead
											data-testid="table-header-cold-start"
											className="min-w-28 text-right font-semibold text-foreground"
										>
								...
								{(metricView === "memory" || metricView === "all") && (
									<>
										<TableHead
											data-testid="table-header-memory-p50"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="memoryP50" label="Memory p50 (MB)" />
										</TableHead>
										<TableHead
											data-testid="table-header-memory-p95"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="memoryP95" label="Memory p95 (MB)" />
										</TableHead>
										<TableHead
											data-testid="table-header-memory-p99"
											className="min-w-28 text-right font-semibold text-foreground"
										>
											<SortButton field="memoryP99" label="Memory p99 (MB)" />
										</TableHead>
										<TableHead
											data-testid="table-header-disk-size"
											className="min-w-24 text-right font-semibold text-foreground"
										>
								...
								{metricView !== "all" && (
									<TableHead
										data-testid="table-header-success-rate"
										className="min-w-28 text-right font-semibold text-foreground"
									>
							...
							{paginatedData.length === 0 ? (
								<TableRow>
									<TableCell colSpan={metricView === "all" ? 15 : 8} className="text-center text-muted-foreground py-8">
										{totalRows === 0 ? "No benchmark data available" : "No results match your filter criteria"}
									</TableCell>
								</TableRow>
							) : (
								paginatedData.map((row, index) => (
									...
									>
										...
										<TableCell data-testid={`cell-ocrMode-${row.key}`}>
											{row.ocrMode === "no_ocr" ? "No OCR" : "With OCR"}
										</TableCell>
