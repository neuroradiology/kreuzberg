...
import { filterFrameworksByFileType, getFrameworkCapabilities } from "@/utils/frameworkCapabilities";
...
export function PerformanceChartsPage() {
	...
	const { filteredData, frameworkSupport } = useMemo(() => {
		...
		const supportedFrameworkKeys = filterFrameworksByFileType(allFrameworkKeys, selectedFileType, capabilities);
	...
	return (
		<div data-testid="page-charts" className="container mx-auto p-4">
			...
			<div className="mb-6 flex gap-4">
				...
				/>
				<OCRModeFilter selectedOCRMode={ocrMode} onOCRModeChange={setOcrMode} data-testid="filter-ocr" />
			</div>

			{!hasRequiredFilters ? (
				<Alert data-testid="validation-message">
					<AlertDescription>Select a file type and OCR mode to view charts</AlertDescription>
				</Alert>
			) : (
				<>
					{hasFilteredFrameworks && (
						<Alert className="mb-6" data-testid="framework-filter-indicator">
							<AlertDescription>
								Showing {frameworkSupport.supporting} of {frameworkSupport.total} frameworks that support{" "}
								{selectedFileType.toUpperCase()}
							</AlertDescription>
						</Alert>
					)}
					<div className="space-y-6">
						<ThroughputChart framework={selectedFramework} fileType={selectedFileType} ocrMode={ocrMode} />

						<MemoryChart framework={selectedFramework} fileType={selectedFileType} ocrMode={ocrMode} />

						<DurationChart fileType={selectedFileType} ocrMode={ocrMode} />

						<ColdStartChart framework={selectedFramework} />
