...
async fn test_orgmode_basic_extraction() {
    ...
    let result = extract_bytes_result(&content, "text/x-org", &ExtractionConfig::default())
        .await
...
"#;
    ...
    let result = extract_bytes_result(org_content.as_bytes(), "text/x-org", &ExtractionConfig::default())
        .await
...
async fn test_orgmode_empty_document() {
    ...
    let result = extract_bytes_result(empty_org.as_bytes(), "text/x-org", &ExtractionConfig::default())
        .await
...
"#;
    ...
    let result = extract_bytes_result(metadata_only.as_bytes(), "text/x-org", &ExtractionConfig::default())
        .await
...
"#;
    ...
    let result = extract_bytes_result(deep_org.as_bytes(), "text/x-org", &ExtractionConfig::default())
        .await
...
async fn test_orgmode_extraction_statistics() {
    ...
    for orgmode_file in test_files {
        ...
        match std::fs::read(&test_file) {
            Ok(content) => match extract_bytes_result(&content, "text/x-org", &ExtractionConfig::default()).await {
                Ok(result) => {
                    total_files += 1;
