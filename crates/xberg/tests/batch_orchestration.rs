...
mod helpers;
use helpers::{
    BytesBatchInput, FileBatchInput, extract_bytes_batch_results, extract_file_batch_results, extract_file_result,
    extract_file_result_blocking,
};
...
async fn test_batch_documents_parallel_execution() {
    ...
    let results = extract_file_batch_results(
        paths
            .clone()
            .into_iter()
...
async fn test_batch_documents_preserves_order() {
    ...
    let results = extract_file_batch_results(paths, &config)
        .await
...
async fn test_multipage_pdf_extraction() {
    ...
    let start = Instant::now();
    let result = extract_file_result(&pdf_path, None, &config).await;
    let duration = start.elapsed();
...
async fn test_concurrent_pdf_extractions() {
    ...
    let mut paths: Vec<FileBatchInput> = Vec::new();
    for _ in 0..10 {
        paths.push(FileBatchInput {
...
fn test_ocr_multipage_efficiency() {
    ...
    let start = Instant::now();
    let result1 = extract_file_result_blocking(&file_path, None, &config);
    let first_duration = start.elapsed();

    assert!(result1.is_ok(), "First OCR should succeed");

    let start = Instant::now();
    let result2 = extract_file_result_blocking(&file_path, None, &config);
...
///
/// Validates that extract_bytes_batch_results processes data in parallel.
#[tokio::test]
async fn test_batch_bytes_parallel_processing() {
    ...
    let owned_contents: Vec<BytesBatchInput> = contents_ref
        .into_iter()
...
async fn test_batch_scales_with_cpu_count() {
    ...
    let owned_contents_1: Vec<BytesBatchInput> = contents_ref
        .iter()
        .map(|(bytes, mime)| BytesBatchInput {
            content: bytes.to_vec(),
            mime_type: mime.to_string(),
    ...
    let start = Instant::now();
    let _ = extract_bytes_batch_results(owned_contents_1, &config_1)
        .await
        .expect("Async operation failed");
    ...
    let owned_contents_full: Vec<BytesBatchInput> = contents_ref
        .into_iter()
        .map(|(bytes, mime)| BytesBatchInput {
            content: bytes.to_vec(),
            mime_type: mime.to_string(),
    ...
    let start = Instant::now();
    let _ = extract_bytes_batch_results(owned_contents_full, &config_full)
        .await
        .expect("Async operation failed");
...
async fn test_batch_accuracy_under_load() {
    ...
    let results = extract_bytes_batch_results(owned_contents, &config)
        .await
        .expect("Async operation failed");
