...
//!
//! These exercise the public `extract_bytes_result_blocking` API to confirm:
...
fn test_extract_tables_flag_false_suppresses_all_tables() {
    ...
    let result = extract_bytes_result_blocking(&bytes, PDF_MIME, &config).expect("extraction must succeed");
    assert!(
...
fn test_minimal_pdf_does_not_panic() {
    ...
    let config = ExtractionConfig::default();
    let _ = extract_bytes_result_blocking(&bytes, PDF_MIME, &config).expect("extraction must succeed");
}
...
/// this test exercises the full public API path: `extract_bytes_result_blocking` with default config.
#[test]
