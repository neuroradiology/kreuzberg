...
async fn test_output_format_plain_produces_plain() {
    ...
    let result = extract_bytes_result(plain_text, "text/plain", &config)
        .await
...
async fn test_output_format_markdown_produces_markdown() {
    ...
    let result = extract_bytes_result(html, "text/html", &config)
        .await
        .expect("Should extract successfully");
...
async fn test_output_format_html_produces_html() {
    ...
    let result = extract_bytes_result(text.as_bytes(), "text/plain", &config)
        .await
        .expect("Should extract successfully");
...
async fn test_chunking_max_chars_limits_chunk_size() {
    ...
    let result = extract_bytes_result(long_text.as_bytes(), "text/plain", &config)
        .await
        .expect("Should extract successfully");
...
async fn test_cache_enabled_allows_caching() {
    ...
    let result1 = extract_bytes_result(text.as_bytes(), "text/plain", &config)
        .await
        .expect("First extraction should succeed");

    let result2 = extract_bytes_result(text.as_bytes(), "text/plain", &config)
        .await
...
async fn test_large_document_with_combined_config() {
    ...
    let result = extract_bytes_result(large_text.as_bytes(), "text/plain", &config)
        .await
        .expect("Should extract successfully");
