...
///
/// 1. `extract_bytes_result_blocking` must return `Ok` (not a hard error).
...
fn raster_png_to_svg_target_warns_and_preserves_bytes() {
    let result = extract_bytes_result_blocking(HTML_WITH_PNG_DATA_URI, "text/html", &config_svg_target())
...
fn raster_to_svg_does_not_return_hard_error() {
    let outcome = extract_bytes_result_blocking(HTML_WITH_PNG_DATA_URI, "text/html", &config_svg_target());
    assert!(
        outcome.is_ok(),
