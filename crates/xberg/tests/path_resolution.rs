...
async fn test_markdown_resolves_images() {
    ...
    let result = extract_file_result(&path, None, &config).await.unwrap();
...
async fn test_markdown_bytes_no_resolution() {
    ...
    let config = config_with_images();
    let result = extract_bytes_result(&content, "text/markdown", &config).await.unwrap();

    // extract_bytes_result has no file path context, so no image resolution should happen
    let image_count = result.images.as_ref().map_or(0, |imgs| imgs.len());
    assert_eq!(image_count, 0, "extract_bytes_result should not resolve local images");
}
...
async fn test_rst_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_file_result(&path, Some("text/x-rst"), &config).await.unwrap();
...
async fn test_orgmode_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_file_result(&path, Some("text/x-org"), &config).await.unwrap();
...
async fn test_typst_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_file_result(&path, Some("application/x-typst"), &config)
        .await
        .unwrap();
...
async fn test_djot_resolves_images() {
    ...
    let config = config_with_images();
    let result = extract_file_result(&path, Some("text/djot"), &config).await.unwrap();
...
async fn test_traversal_blocked() {
    ...
    let config = config_with_images();
    let result = extract_file_result(&md_path, Some("text/markdown"), &config)
        .await
        .unwrap();
