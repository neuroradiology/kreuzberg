...
//! public `extract_bytes_result_blocking` entry point and asserts the extraction fails
...
fn xml_depth_bomb_fires_security_error() {
    ...
    let err = extract_bytes_result_blocking(payload.as_bytes(), "application/xml", &cfg)
...
fn xml_benign_input_extracts_successfully() {
    ...
    let cfg = config_with_tight_limits();
    let result = extract_bytes_result_blocking(payload.as_bytes(), "application/xml", &cfg)
