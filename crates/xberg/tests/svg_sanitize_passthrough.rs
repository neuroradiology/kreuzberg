...
fn svg_sanitize_strips_script_and_external_href() {
    let result = extract_bytes_result_blocking(HTML_WITH_XSS_SVG, "text/html", &sanitize_config())
...
fn svg_without_sanitize_preserves_original_bytes_on_native_target() {
    let result = extract_bytes_result_blocking(HTML_WITH_XSS_SVG, "text/html", &no_sanitize_config())
        .expect("extraction must succeed");
