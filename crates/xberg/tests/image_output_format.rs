...
//!
//! Drives real extractors through `extract_file_result_blocking` and asserts that the
...
fn pdf_native_passthrough() {
    ...
    let result_native =
        extract_file_result_blocking(&path, None, &config_explicit_native).expect("native extraction must succeed");
    let result_default =
        extract_file_result_blocking(&path, None, &config_default).expect("default extraction must succeed");
...
fn pdf_force_png() {
    ...
    let result = extract_file_result_blocking(&path, None, &config).expect("PNG extraction must succeed");
...
fn pdf_force_jpeg() {
    ...
    let result = extract_file_result_blocking(&path, None, &config).expect("JPEG extraction must succeed");
...
fn pdf_force_webp() {
    ...
    let result = extract_file_result_blocking(&path, None, &config).expect("WebP extraction must succeed");
...
fn office_mixed_to_png() {
    ...
    let config = config_with_output_format(ImageOutputFormat::Png);
    let result = extract_file_result_blocking(&path, None, &config).expect("DOCX PNG extraction must succeed");
...
fn image_extractor_jpeg_to_png() {
    ...
    let native_result =
        extract_file_result_blocking(&path, None, &native_config).expect("native JPEG extraction must succeed");

    // Extract with PNG re-encode.
    let png_config = config_with_output_format(ImageOutputFormat::Png);
    let png_result = extract_file_result_blocking(&path, None, &png_config).expect("PNG JPEG extraction must succeed");
...
fn ocr_runs_before_reencode() {
    ...
    let result = extract_file_result_blocking(&path, None, &config).expect("OCR + JPEG re-encode must succeed");
...
fn quality_change_alters_byte_size() {
    ...
    let result_high =
        extract_file_result_blocking(&path, None, &config_high).expect("high-quality JPEG extraction must succeed");
    let result_low =
        extract_file_result_blocking(&path, None, &config_low).expect("low-quality JPEG extraction must succeed");
...
fn unsupported_source_format_warns_and_preserves() {
    ...
    let config = config_with_output_format(ImageOutputFormat::Png);
    let result = extract_file_result_blocking(&path, None, &config).expect("DOCX extraction must not error out");
...
fn heif_output() {
    ...
    let result = extract_file_result_blocking(&path, None, &config).expect("HEIF extraction must succeed");
