...
async fn test_rst_title_extraction() {
    ...
    let result = extract_bytes_result(&content, "text/x-rst", &ExtractionConfig::default())
        .await
...
async fn test_rst_extraction_no_errors() {
    ...
    let result = extract_bytes_result(&content, "text/x-rst", &ExtractionConfig::default()).await;
