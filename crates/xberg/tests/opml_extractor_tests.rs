...
async fn test_opml_rss_feeds_extraction() {
    ...
    let result = extract_bytes_result(&content, "text/x-opml", &ExtractionConfig::default())
        .await
...
async fn test_opml_mime_type_handling() {
    ...
    let result2 = extract_bytes_result(&content, "application/xml+opml", &ExtractionConfig::default())
        .await
...
async fn test_opml_extraction_statistics() {
    ...
    for opml_file in opml_files {
        ...
        match std::fs::read(&test_file) {
            Ok(content) => match extract_bytes_result(&content, "text/x-opml", &ExtractionConfig::default()).await {
                Ok(result) => {
                    total_files += 1;
