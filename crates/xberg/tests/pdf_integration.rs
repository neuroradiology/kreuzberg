...
fn test_corrupted_pdf_returns_error_not_panic() {
    ...
    let result = extract_bytes_result_blocking(b"not a pdf", "application/pdf", &config);
    assert!(result.is_err(), "Garbage bytes should return Err, not Ok");

    // Truncated PDF header with no content.
    let result = extract_bytes_result_blocking(b"%PDF-1.4\n%%EOF", "application/pdf", &config);
    ...
    let result = extract_bytes_result_blocking(&noisy, "application/pdf", &config);
