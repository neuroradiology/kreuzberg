...
async fn test_csv_basic_extraction() {
    ...
    let extraction = match extract_bytes_result(csv_content, "text/csv", &config).await {
        Ok(result) => result,
        Err(_) => {
...
async fn test_tsv_file() {
    ...
    let extraction = match extract_bytes_result(tsv_content, "text/tab-separated-values", &config).await {
        Ok(result) => result,
        Err(_) => {
...
async fn test_csv_large_file() {
    ...
    let extraction = match extract_bytes_result(csv_content.as_bytes(), "text/csv", &config).await {
        Ok(result) => result,
        Err(_) => {
            println!("Skipping test: CSV extraction not available");
            return;
...
async fn test_csv_malformed() {
    ...
    let result = extract_bytes_result(csv_content, "text/csv", &config).await;
...
async fn test_csv_empty() {
    ...
    let result = extract_bytes_result(empty_csv, "text/csv", &config).await;
