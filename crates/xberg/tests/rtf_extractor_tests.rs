...
async fn test_rtf_accent_extraction() {
    ...
    let result = extract_file_result(&path, Some("application/rtf"), &config).await;
...
async fn test_rtf_extraction_deterministic_unicode() {
    ...
    let result1 = extract_file_result(&path, Some("application/rtf"), &config).await;
    let result2 = extract_file_result(&path, Some("application/rtf"), &config).await;
...
async fn test_rtf_word_sample_matches_docx_metadata_and_content() {
    ...
    let config = ExtractionConfig::default();
    let rtf_result = extract_file_result(&rtf_path, Some("application/rtf"), &config)
        .await
        .expect("RTF extraction should succeed for word_sample");
    let docx_result = extract_file_result(&docx_path, None, &config)
        .await
...
async fn test_rtf_lorem_ipsum_metadata_alignment() {
    ...
    let config = ExtractionConfig::default();
    let result = extract_file_result(&rtf_path, Some("application/rtf"), &config)
        .await
...
async fn test_rtf_comprehensive_extraction_alignment() {
    ...
    let odt_result = extract_file_result(&odt_path, None, &config)
        .await
