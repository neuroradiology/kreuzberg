...
async fn test_pst_invalid_data_returns_error() {
    ...
    let result = extract_bytes_result(b"not a pst file", "application/vnd.ms-outlook-pst", &config).await;
...
async fn test_pst_empty_file_extraction() {
    ...
    let result = extract_bytes_result(&pst_bytes, "application/vnd.ms-outlook-pst", &config)
        .await
