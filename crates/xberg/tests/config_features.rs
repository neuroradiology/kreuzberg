...
async fn test_chunking_enabled() {
    ...
    let result = extract_bytes_result(text_bytes, "text/plain", &config)
        .await
...
async fn test_cache_hit_behavior() {
    ...
    let result1 = extract_bytes_result(text_bytes, "text/plain", &config)
        .await
        .expect("First extraction should succeed");

    let result2 = extract_bytes_result(text_bytes, "text/plain", &config)
        .await
...
async fn test_cache_miss_invalidation() {
    ...
    let result1 = extract_bytes_result(text1.as_bytes(), "text/plain", &config)
        .await
        .expect("First extraction should succeed");

    let result2 = extract_bytes_result(text2.as_bytes(), "text/plain", &config)
        .await
        .expect("Second extraction should succeed");
...
async fn test_quality_threshold_filtering() {
    ...
    let result_high = extract_bytes_result(high_quality.as_bytes(), "text/plain", &config)
        .await
        .expect("Should extract successfully");

    let low_quality = "a  b  c  d  ....... word123mixed .  . ";
    let result_low = extract_bytes_result(low_quality.as_bytes(), "text/plain", &config)
        .await
        .expect("Should extract successfully");
...
"#;
    ...
    let result = extract_bytes_result(markdown.as_bytes(), "text/markdown", &config)
        .await
        .expect("Should extract successfully");
