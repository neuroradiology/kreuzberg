...
async fn test_extract_bytes_zero_timeout_returns_timeout_error() {
    ...
    let result = extract_bytes_result(content, "text/plain", &config).await;
...
/// Same check for extract_file_result.
#[cfg(feature = "tokio-runtime")]
#[tokio::test]
...
async fn test_extract_bytes_timeout_elapsed_is_plausible() {
    ...
    let _ = extract_bytes_result(content, "text/plain", &config).await;
