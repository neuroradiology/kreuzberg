...
async fn test_truncated_pdf() {
    ...
    let result = extract_bytes_result(truncated_pdf, "application/pdf", &config).await;
...
async fn test_corrupted_zip() {
    ...
    let result = extract_bytes_result(&corrupted_zip, "application/zip", &config).await;
...
<another>text</wrong_tag>\n\
    ...
    let result = extract_bytes_result(invalid_xml, "application/xml", &config).await;
...
async fn test_corrupted_image() {
    ...
    let result = extract_bytes_result(&corrupted_png, "image/png", &config).await;
...
async fn test_empty_file() {
    ...
    let result_text = extract_bytes_result(empty_data, "text/plain", &config).await;
    #[cfg(feature = "pdf")]
    {
        let result_pdf = extract_bytes_result(empty_data, "application/pdf", &config).await;
        match result_pdf {
            Ok(extraction) => {
                assert!(
    ...
    #[cfg(feature = "xml")]
    {
        let result_xml = extract_bytes_result(empty_data, "application/xml", &config).await;
        match result_xml {
            Ok(extraction) => {
                assert!(
                    extraction.content.is_empty(),
...
async fn test_very_large_file() {
    ...
    let result = extract_bytes_result(large_bytes, "text/plain", &config).await;
...
async fn test_unicode_filenames() {
    ...
    let result = extract_file_result(temp_file.path(), Some("text/plain"), &config).await;
...
Math symbols: ∑ ∫ √ ≈ ∞";
    ...
    let result = extract_bytes_result(special_text.as_bytes(), "text/plain", &config).await;
...
async fn test_nonexistent_file() {
    ...
    let result = extract_file_result(nonexistent_path, Some("application/pdf"), &config).await;
...
async fn test_unsupported_format() {
    ...
    let result = extract_bytes_result(data, "application/x-unknown-format", &config).await;
...
async fn test_file_extension_mismatch() {
    ...
    let result = extract_bytes_result(&docx_magic, "application/pdf", &config).await;
...
async fn test_null_bytes_in_content() {
    ...
    let result = extract_bytes_result(data_with_nulls, "text/plain", &config).await;
...
async fn test_concurrent_extractions() {
    ...
    let handles: Vec<_> = (0..10)
        .map(|_| {
            ...
            tokio::spawn(async move { extract_bytes_result(text_data, "text/plain", &config).await })
        })
        .collect();
