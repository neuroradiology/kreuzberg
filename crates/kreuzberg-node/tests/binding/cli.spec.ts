...
describe("CLI", () => {
	...
	describe("CLI binary detection", () => {
		it("should find kreuzberg-cli 4.0.7 when available", () => {
		...
		it("should handle missing kreuzberg-cli 4.0.7", () => {
			const mockWhichSync = vi.mocked(which.sync);
	...
	describe("CLI argument passing", () => {
		it("should pass arguments to kreuzberg-cli 4.0.7", () => {
			const mockWhichSync = vi.mocked(which.sync);
			const mockSpawnSync = vi.mocked(spawnSync);
		...
		it("should handle --version flag", () => {
			...
			mockSpawnSync.mockReturnValue({
				...
				pid: 12345,
				output: [],
				stdout: Buffer.from("kreuzberg-cli 4.0.7"),
				stderr: Buffer.from(""),
				signal: null,
			});
