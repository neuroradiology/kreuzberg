...
pub struct JsUrlExtractionConfig {
    ...
    #[napi(js_name = "requestTimeoutSecs")]
    ...
    #[napi(js_name = "documentMaxSizeBytes")]
    #[serde(rename = "documentMaxSizeBytes")]
    pub document_max_size_bytes: Option<i64>,
    /// MIME allowlist for downloaded documents.
    #[napi(js_name = "allowedFileTypes")]
...
pub fn extract_file_sync(
    ...
    xberg::core::extractor::extract_file_sync(std::path::PathBuf::from(path), mime_type.as_deref(), &config_core)
        .map(|val| val.into())
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))
}
...
pub fn extract_bytes_sync(
    ...
    let content: Vec<u8> = content.to_vec();
    xberg::core::extractor::extract_bytes_sync(&content, &mime_type, &config_core)
        .map(|val| val.into())
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))
}
...
pub fn batch_extract_files_sync(
    ...
    xberg::core::extractor::batch_extract_files_sync(items_core, &config_core)
...
pub fn batch_extract_bytes_sync(
    ...
    xberg::core::extractor::batch_extract_bytes_sync(items_core, &config_core)
        .map(|val| val.into_iter().map(Into::into).collect())
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))
}
...
pub async fn batch_extract_files(
    ...
    let result = xberg::core::extractor::batch_extract_files(items_core, &config_core)
        .await
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))?;
    Ok(result.into_iter().map(Into::into).collect())
}
...
pub async fn batch_extract_bytes(
    ...
    let result = xberg::core::extractor::batch_extract_bytes(items_core, &config_core)
        .await
        .map_err(|e| napi::Error::new(napi::Status::GenericFailure, e.to_string()))?;
    Ok(result.into_iter().map(Into::into).collect())
}
...
impl From<JsUrlExtractionConfig> for xberg::UrlExtractionConfig {
    fn from(val: JsUrlExtractionConfig) -> Self {
        ...
        __result.request_timeout_secs = val.request_timeout_secs.map(|v| v as u64);
        if let Some(__v) = val.download_documents {
            __result.download_documents = __v;
        }
        __result.document_max_size_bytes = val.document_max_size_bytes.map(|v| v as u64);
        if let Some(__v) = val.allowed_file_types {
            __result.allowed_file_types = __v.into_iter().collect();
        }
...
impl From<xberg::UrlExtractionConfig> for JsUrlExtractionConfig {
    fn from(val: xberg::UrlExtractionConfig) -> Self {
        Self {
            ...
            allowed_file_types: Some(val.allowed_file_types.into_iter().collect()),
