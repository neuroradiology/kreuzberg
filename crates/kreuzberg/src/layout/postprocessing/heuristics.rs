...
pub(crate) fn apply_heuristics(
    ...
    for d in detections.iter_mut() {
        if matches!(d.class_name, LayoutClass::Table | LayoutClass::Picture | LayoutClass::Chart)
...
fn pick_removal(a: &LayoutDetection, b: &LayoutDetection, containment_a_of_b: f32) -> usize {
    ...
    if a.class_name == LayoutClass::Text
        && matches!(b.class_name, LayoutClass::Table | LayoutClass::Picture | LayoutClass::Chart)
        && a.confidence >= b.confidence
    {
        return 1; // remove Table/Picture/Chart, keep Text
    }
    if b.class_name == LayoutClass::Text
        && matches!(a.class_name, LayoutClass::Table | LayoutClass::Picture | LayoutClass::Chart)
