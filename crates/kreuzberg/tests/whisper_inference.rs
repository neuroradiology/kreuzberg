...
//!
//! ```text
//! cargo test -p kreuzberg --features transcription --test whisper_inference
//! ```
...
use kreuzberg::core::config::transcription::WhisperModel;
use kreuzberg::transcription::decode::decode_audio_to_pcm;
use kreuzberg::transcription::engine::WhisperEngine;
use kreuzberg::transcription::model::ensure_whisper_model;
...
fn empty_pcm_returns_empty_string() {
    use kreuzberg::transcription::decode::PcmAudio;
    let engine = load_tiny();
    let pcm = PcmAudio {
        samples: Vec::new(),
