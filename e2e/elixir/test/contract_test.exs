# This file is auto-generated by alef — DO NOT EDIT.
# alef:hash:c39dc9eda1096e5908f5d3623329d139e738e2f60f478cd21a1d3d23c7132388
# To regenerate: alef generate
# To verify freshness: alef verify --exit-code
# E2e tests for category: contract
defmodule E2e.ContractTest do
  use ExUnit.Case, async: false

  defp alef_e2e_format_to_string(value) when is_binary(value), do: value
  defp alef_e2e_format_to_string(metadata) do
    case metadata.image do
      %{format: fmt} when is_binary(fmt) -> fmt
      _ ->
        case metadata.pdf do
          %{} -> "PDF"
          _ ->
            case metadata.html do
              %{} -> "HTML"
              _ -> inspect(metadata)
            end
        end
    end
  end

  describe "api_batch_bytes_async" do
    test "api_batch_bytes_async" do
      {:ok, result} = Xberg.extract_async("{\"path\":\"pdf/fake_memo.pdf\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert Enum.any?(["May 5, 2023", "Mallori"], fn v -> String.contains?(to_string(result.content), v) end)
    end
  end

  describe "api_batch_bytes_with_configs_async" do
    test "api_batch_bytes_with_configs_async" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"output_format\":\"markdown\"},\"path\":\"pdf/fake_memo.pdf\"}", "{\"output_format\":\"markdown\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert String.trim(result.metadata.output_format) == "markdown"
    end
  end

  describe "api_batch_file_async" do
    test "api_batch_file_async" do
      {:ok, result} = Xberg.extract_async("{\"path\":\"pdf/fake_memo.pdf\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert Enum.any?(["May 5, 2023", "Mallori"], fn v -> String.contains?(to_string(result.content), v) end)
    end
  end

  describe "api_batch_file_with_configs_async" do
    test "api_batch_file_with_configs_async" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"output_format\":\"markdown\"},\"path\":\"pdf/fake_memo.pdf\"}", "{\"output_format\":\"markdown\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert String.trim(result.metadata.output_format) == "markdown"
    end
  end

  describe "api_extract_bytes_async" do
    test "api_extract_bytes_async" do
      {:ok, result} = Xberg.extract_async("{\"path\":\"pdf/fake_memo.pdf\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert Enum.any?(["May 5, 2023", "Mallori"], fn v -> String.contains?(to_string(result.content), v) end)
    end
  end

  describe "api_extract_file_async" do
    test "api_extract_file_async" do
      {:ok, result} = Xberg.extract_async("{\"path\":\"pdf/fake_memo.pdf\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert Enum.any?(["May 5, 2023", "Mallori"], fn v -> String.contains?(to_string(result.content), v) end)
    end
  end

  describe "config_document_structure_with_headings" do
    test "config_document_structure_with_headings" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"include_document_structure\":true},\"path\":\"docx/fake.docx\"}", "{\"include_document_structure\":true}")
      assert String.trim(result.mime_type) == "application/vnd.openxmlformats-officedocument.wordprocessingml.document"
      assert result.document != ""
      assert length(result.document.nodes) >= 1
    end
  end

  describe "config_element_types" do
    test "config_element_types" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"result_format\":\"element_based\"},\"path\":\"docx/unit_test_headers.docx\"}", "{\"result_format\":\"element_based\"}")
      assert Enum.any?(["application/vnd.openxmlformats-officedocument.wordprocessingml.document"], fn v -> String.contains?(to_string(result.mime_type), v) end)
      assert length(result.elements) >= 1
    end
  end

  describe "config_extraction_timeout" do
    test "config_extraction_timeout" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"extraction_timeout_secs\":300},\"path\":\"pdf/fake_memo.pdf\"}", "{\"extraction_timeout_secs\":300}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
    end
  end

  describe "config_keywords" do
    test "config_keywords" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"keywords\":{\"algorithm\":\"yake\",\"max_keywords\":10}},\"path\":\"pdf/fake_memo.pdf\"}", "{\"keywords\":{\"algorithm\":\"yake\",\"max_keywords\":10}}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      # skipped: field 'keywords' not available on Elixir result type
      # skipped: field 'keywords' not available on Elixir result type
    end
  end

  describe "config_pages" do
    test "config_pages" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"pages\":{\"extract_pages\":true,\"insert_page_markers\":true}},\"path\":\"pdf/fake_memo.pdf\"}", "{\"pages\":{\"extract_pages\":true,\"insert_page_markers\":true}}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert Enum.any?(["PAGE"], fn v -> String.contains?(to_string(result.content), v) end)
    end
  end

  describe "config_quality_enabled" do
    test "config_quality_enabled" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"enable_quality_processing\":true},\"path\":\"pdf/fake_memo.pdf\"}", "{\"enable_quality_processing\":true}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert result.quality_score != ""
      assert result.quality_score >= 0.0
      assert result.quality_score <= 1.0
    end
  end

  describe "config_security_limits" do
    test "config_security_limits" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"security_limits\":{\"max_archive_size\":104857600,\"max_compression_ratio\":50,\"max_files_in_archive\":100}},\"path\":\"archives/documents.zip\"}", "{\"security_limits\":{\"max_archive_size\":104857600,\"max_compression_ratio\":50,\"max_files_in_archive\":100}}")
      assert Enum.any?(["application/zip", "application/x-zip-compressed"], fn v -> String.contains?(to_string(result.mime_type), v) end)
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
    end
  end

  describe "config_tree_sitter" do
    test "config_tree_sitter" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"tree_sitter\":{\"groups\":[\"web\"],\"languages\":[\"python\",\"rust\"],\"process\":{\"comments\":false,\"diagnostics\":false,\"docstrings\":false,\"exports\":true,\"imports\":true,\"structure\":true,\"symbols\":false}}},\"path\":\"code/hello.py\"}", "{\"tree_sitter\":{\"groups\":[\"web\"],\"languages\":[\"python\",\"rust\"],\"process\":{\"comments\":false,\"diagnostics\":false,\"docstrings\":false,\"exports\":true,\"imports\":true,\"structure\":true,\"symbols\":false}}}")
      assert String.trim(result.mime_type) == "text/x-source-code"
      assert (is_binary(result.content) && byte_size(result.content) >= 5) || (is_list(result.content) && length(result.content) >= 5)
    end
  end

  describe "output_format_bytes_markdown" do
    test "output_format_bytes_markdown" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"output_format\":\"markdown\"},\"data\":\"pdf/fake_memo.pdf\",\"mime_type\":\"application/pdf\"}", "{\"output_format\":\"markdown\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert String.trim(result.metadata.output_format) == "markdown"
    end
  end

  describe "output_format_markdown" do
    test "output_format_markdown" do
      {:ok, result} = Xberg.extract_async("{\"config\":{\"output_format\":\"markdown\"},\"path\":\"pdf/fake_memo.pdf\"}", "{\"output_format\":\"markdown\"}")
      assert String.trim(result.mime_type) == "application/pdf"
      assert (is_binary(result.content) && byte_size(result.content) >= 10) || (is_list(result.content) && length(result.content) >= 10)
      assert String.trim(result.metadata.output_format) == "markdown"
    end
  end
end
