...
describe("summarization", () => {
	...
	it("summarization_extractive_smoke: TextRank extractive summary over a multi-paragraph plain text document. Pure-Rust, deterministic, no external services required.", async () => {
		const result = await extract({ config: { summarization: { maxTokens: 80, strategy: "extractive" } }, path: "text/book_war_and_peace_1p.txt" } as ExtractionConfig, { summarization: { maxTokens: 80, strategy: "extractive" } } as ExtractionConfig);
