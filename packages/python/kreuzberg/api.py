...
async def summarize_with_llm(
    ...
    return await _rust.summarize_with_llm(text=text, llm_config=_rust_llm_config, max_tokens=max_tokens)
...
async def extract_region_with_vlm_usage(
    image_bytes: bytes,
    image_mime: str,
    region_kind: RegionKind,
    llm_config: LlmConfig | None = None,
    ...
    """Same as [`extract_region_with_vlm`], but also returns the [`LlmUsage`] data captured from."""
    _rust_llm_config = _to_rust_llm_config(llm_config)

    if _rust_llm_config is None:
        _rust_llm_config = _rust.LlmConfig()
    return await _rust.extract_region_with_vlm_usage(image_bytes=image_bytes, image_mime=image_mime, region_kind=region_kind, llm_config=_rust_llm_config, custom_prompt=custom_prompt)
...
async def complete_with_json_schema(
    ...
    """Send a free-form prompt to the configured LLM with a JSON-schema response constraint and ."""
    _rust_llm_config = _to_rust_llm_config(llm_config)

    if _rust_llm_config is None:
        _rust_llm_config = _rust.LlmConfig()
    return await _rust.complete_with_json_schema(llm_config=_rust_llm_config, prompt=prompt, schema_name=schema_name, schema=schema, source=source)


async def complete_text(llm_config: LlmConfig | None = None, prompt: str, source: str) -> str:
    """Send a single user prompt to the configured LLM and return the response text along with t."""
    _rust_llm_config = _to_rust_llm_config(llm_config)

    if _rust_llm_config is None:
        _rust_llm_config = _rust.LlmConfig()
    return await _rust.complete_text(llm_config=_rust_llm_config, prompt=prompt, source=source)
