...
pub struct UrlExtractionConfig {
    ...
    pub document_max_size_bytes: Option<f64>,
...
pub fn new_urlextractionconfig(
    ...
    document_max_size_bytes: Option<f64>,
    allowed_file_types: Option<Vec<String>>,
    follow_document_urls: Option<bool>,
    document_url_depth: Option<i32>,
    ...
    if let Some(v) = max_pages {
        __out.max_pages = Some(v);
    }
    if let Some(v) = max_concurrent {
        __out.max_concurrent = Some(v);
    }
    if let Some(v) = stay_on_domain {
        __out.stay_on_domain = v;
    }
    if let Some(v) = allow_subdomains {
        __out.allow_subdomains = v;
    }
    if let Some(v) = include_paths {
        __out.include_paths = v;
    }
    if let Some(v) = exclude_paths {
        __out.exclude_paths = v;
    }
    if let Some(v) = respect_robots_txt {
        __out.respect_robots_txt = v;
    }
    if let Some(v) = user_agent {
        __out.user_agent = Some(v);
    }
    if let Some(v) = request_timeout_secs {
        __out.request_timeout_secs = Some(v);
    }
    if let Some(v) = download_documents {
        __out.download_documents = v;
    }
    if let Some(v) = document_max_size_bytes {
        __out.document_max_size_bytes = Some(v);
    }
    if let Some(v) = allowed_file_types {
        __out.allowed_file_types = v;
    }
    if let Some(v) = follow_document_urls {
        __out.follow_document_urls = v;
    }
    if let Some(v) = document_url_depth {
        __out.document_url_depth = Some(v);
    }
...
impl From<UrlExtractionConfig> for xberg::UrlExtractionConfig {
    fn from(val: UrlExtractionConfig) -> Self {
        Self {
            mode: val.mode.into(),
            max_depth: val.max_depth.map(|v| v as u32),
            max_pages: val.max_pages.map(|v| v as u32),
            max_concurrent: val.max_concurrent.map(|v| v as u32),
...
impl From<xberg::UrlExtractionConfig> for UrlExtractionConfig {
    fn from(val: xberg::UrlExtractionConfig) -> Self {
        Self {
            mode: val.mode.into(),
            max_depth: val.max_depth.map(|v| v as i32),
            max_pages: val.max_pages.map(|v| v as i32),
            max_concurrent: val.max_concurrent.map(|v| v as i32),
            stay_on_domain: val.stay_on_domain,
            allow_subdomains: val.allow_subdomains,
            include_paths: val.include_paths.into_iter().collect(),
            exclude_paths: val.exclude_paths.into_iter().collect(),
            ...
            request_timeout_secs: val.request_timeout_secs.map(|v| v as f64),
            download_documents: val.download_documents,
            document_max_size_bytes: val.document_max_size_bytes.map(|v| v as f64),
            allowed_file_types: val.allowed_file_types.into_iter().collect(),
            follow_document_urls: val.follow_document_urls,
...
pub fn extract_bytes(content: Vec<u8>, mime_type: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let result = rt.block_on(async {
            xberg::core::extractor::extract_bytes(&content, &mime_type, &config_core)
                .await
                .map_err(|e| {
                    extendr_api::Error::Other(
                        e.to_string()
...
pub fn extract_file(path: String, mime_type: Option<String>, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let result = rt.block_on(async {
            xberg::core::extractor::extract_file(std::path::PathBuf::from(path), mime_type.as_deref(), &config_core)
                .await
                .map_err(|e| {
                    extendr_api::Error::Other(
                        e.to_string()
...
pub fn extract_bytes_sync(content: Vec<u8>, mime_type: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let result = xberg::core::extractor::extract_bytes_sync(&content, &mime_type, &config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
pub fn batch_extract_files_sync(items: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let result = xberg::core::extractor::batch_extract_files_sync(items_core, &config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
pub fn batch_extract_bytes_sync(items: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let config_core: xberg::ExtractionConfig = (*config).clone().into();
        let result = xberg::core::extractor::batch_extract_bytes_sync(items_core, &config_core).map_err(|e| {
            extendr_api::Error::Other(
                e.to_string()
                    .replace(":", "_")
                    .replace("/", "_")
...
pub fn batch_extract_files(items: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let result = rt.block_on(async {
            xberg::core::extractor::batch_extract_files(items_core, &config_core)
                .await
                .map_err(|e| {
                    extendr_api::Error::Other(
                        e.to_string()
...
pub fn batch_extract_bytes(items: String, config: &ExtractionConfig) -> String {
    match (|| -> Result<String> {
        ...
        let result = rt.block_on(async {
            xberg::core::extractor::batch_extract_bytes(items_core, &config_core)
                .await
                .map_err(|e| {
                    extendr_api::Error::Other(
                        e.to_string()
