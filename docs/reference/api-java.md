...
| `maxDepth` | `Optional<Integer>` | `null` | Maximum crawl depth from a seed URL. |
| `maxPages` | `Optional<Integer>` | `null` | Maximum pages to crawl. |
| `maxConcurrent` | `Optional<Integer>` | `null` | Maximum concurrent requests. |
| `stayOnDomain` | `boolean` | `true` | Stay on the seed domain. |
| `allowSubdomains` | `boolean` | `true` | Allow subdomains when staying on domain. |
| `includePaths` | `List<String>` | `Collections.emptyList()` | Regex path include filters. |
| `excludePaths` | `List<String>` | `Collections.emptyList()` | Regex path exclude filters. |
| `respectRobotsTxt` | `boolean` | `true` | Respect robots.txt. |
| `userAgent` | `Optional<String>` | `null` | Custom user agent. |
| `requestTimeoutSecs` | `Optional<Long>` | `null` | Per-request timeout in seconds. |
| `downloadDocuments` | `boolean` | `true` | Download non-HTML documents discovered during crawl. |
| `documentMaxSizeBytes` | `Optional<Long>` | `null` | Maximum size for downloaded documents. |
| `allowedFileTypes` | `List<String>` | `Collections.emptyList()` | MIME allowlist for downloaded documents. |
| `followDocumentUrls` | `boolean` | `false` | Follow URLs discovered inside extracted documents. |
| `documentUrlDepth` | `Optional<Integer>` | `null` | Maximum recursive document URL depth. |
