...
| `maxDepth` | `number \| null` | `null` | Maximum crawl depth from a seed URL. |
| `maxPages` | `number \| null` | `null` | Maximum pages to crawl. |
| `maxConcurrent` | `number \| null` | `null` | Maximum concurrent requests. |
| `stayOnDomain` | `boolean` | `true` | Stay on the seed domain. |
| `allowSubdomains` | `boolean` | `true` | Allow subdomains when staying on domain. |
| `includePaths` | `Array<string>` | `\[\]` | Regex path include filters. |
| `excludePaths` | `Array<string>` | `\[\]` | Regex path exclude filters. |
| `respectRobotsTxt` | `boolean` | `true` | Respect robots.txt. |
| `userAgent` | `string \| null` | `null` | Custom user agent. |
| `requestTimeoutSecs` | `number \| null` | `null` | Per-request timeout in seconds. |
| `downloadDocuments` | `boolean` | `true` | Download non-HTML documents discovered during crawl. |
| `documentMaxSizeBytes` | `number \| null` | `null` | Maximum size for downloaded documents. |
| `allowedFileTypes` | `Array<string>` | `\[\]` | MIME allowlist for downloaded documents. |
| `followDocumentUrls` | `boolean` | `false` | Follow URLs discovered inside extracted documents. |
| `documentUrlDepth` | `number \| null` | `null` | Maximum recursive document URL depth. |
