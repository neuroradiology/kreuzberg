...
| `max_depth` | `Integer?` | `nil` | Maximum crawl depth from a seed URL. |
| `max_pages` | `Integer?` | `nil` | Maximum pages to crawl. |
| `max_concurrent` | `Integer?` | `nil` | Maximum concurrent requests. |
| `stay_on_domain` | `Boolean` | `true` | Stay on the seed domain. |
| `allow_subdomains` | `Boolean` | `true` | Allow subdomains when staying on domain. |
| `include_paths` | `Array<String>` | `\[\]` | Regex path include filters. |
| `exclude_paths` | `Array<String>` | `\[\]` | Regex path exclude filters. |
| `respect_robots_txt` | `Boolean` | `true` | Respect robots.txt. |
| `user_agent` | `String?` | `nil` | Custom user agent. |
| `request_timeout_secs` | `Integer?` | `nil` | Per-request timeout in seconds. |
| `download_documents` | `Boolean` | `true` | Download non-HTML documents discovered during crawl. |
| `document_max_size_bytes` | `Integer?` | `nil` | Maximum size for downloaded documents. |
| `allowed_file_types` | `Array<String>` | `\[\]` | MIME allowlist for downloaded documents. |
| `follow_document_urls` | `Boolean` | `false` | Follow URLs discovered inside extracted documents. |
| `document_url_depth` | `Integer?` | `nil` | Maximum recursive document URL depth. |
