...
| `maxDepth` | `u32?` | `null` | Maximum crawl depth from a seed URL. |
| `maxPages` | `u32?` | `null` | Maximum pages to crawl. |
| `maxConcurrent` | `u32?` | `null` | Maximum concurrent requests. |
| `stayOnDomain` | `bool` | `true` | Stay on the seed domain. |
| `allowSubdomains` | `bool` | `true` | Allow subdomains when staying on domain. |
| `includePaths` | `\[\]const \[:0\]const u8` | `\[\]` | Regex path include filters. |
| `excludePaths` | `\[\]const \[:0\]const u8` | `\[\]` | Regex path exclude filters. |
| `respectRobotsTxt` | `bool` | `true` | Respect robots.txt. |
| `userAgent` | `\[:0\]const u8?` | `null` | Custom user agent. |
| `requestTimeoutSecs` | `u64?` | `null` | Per-request timeout in seconds. |
| `downloadDocuments` | `bool` | `true` | Download non-HTML documents discovered during crawl. |
| `documentMaxSizeBytes` | `u64?` | `null` | Maximum size for downloaded documents. |
| `allowedFileTypes` | `\[\]const \[:0\]const u8` | `\[\]` | MIME allowlist for downloaded documents. |
| `followDocumentUrls` | `bool` | `false` | Follow URLs discovered inside extracted documents. |
| `documentUrlDepth` | `u32?` | `null` | Maximum recursive document URL depth. |
