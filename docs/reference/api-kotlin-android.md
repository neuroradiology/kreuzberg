...
| `maxDepth` | `Int?` | `null` | Maximum crawl depth from a seed URL. |
| `maxPages` | `Int?` | `null` | Maximum pages to crawl. |
| `maxConcurrent` | `Int?` | `null` | Maximum concurrent requests. |
| `stayOnDomain` | `Boolean` | `true` | Stay on the seed domain. |
| `allowSubdomains` | `Boolean` | `true` | Allow subdomains when staying on domain. |
| `includePaths` | `List<String>` | `\[\]` | Regex path include filters. |
| `excludePaths` | `List<String>` | `\[\]` | Regex path exclude filters. |
| `respectRobotsTxt` | `Boolean` | `true` | Respect robots.txt. |
| `userAgent` | `String?` | `null` | Custom user agent. |
| `requestTimeoutSecs` | `Long?` | `null` | Per-request timeout in seconds. |
| `downloadDocuments` | `Boolean` | `true` | Download non-HTML documents discovered during crawl. |
| `documentMaxSizeBytes` | `Long?` | `null` | Maximum size for downloaded documents. |
| `allowedFileTypes` | `List<String>` | `\[\]` | MIME allowlist for downloaded documents. |
| `followDocumentUrls` | `Boolean` | `false` | Follow URLs discovered inside extracted documents. |
| `documentUrlDepth` | `Int?` | `null` | Maximum recursive document URL depth. |
