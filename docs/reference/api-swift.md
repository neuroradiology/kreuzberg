...
| `maxDepth` | `UInt32?` | `null` | Maximum crawl depth from a seed URL. |
| `maxPages` | `UInt32?` | `null` | Maximum pages to crawl. |
| `maxConcurrent` | `UInt32?` | `null` | Maximum concurrent requests. |
| `stayOnDomain` | `Bool` | `true` | Stay on the seed domain. |
| `allowSubdomains` | `Bool` | `true` | Allow subdomains when staying on domain. |
| `includePaths` | `\[String\]` | `\[\]` | Regex path include filters. |
| `excludePaths` | `\[String\]` | `\[\]` | Regex path exclude filters. |
| `respectRobotsTxt` | `Bool` | `true` | Respect robots.txt. |
| `userAgent` | `String?` | `null` | Custom user agent. |
| `requestTimeoutSecs` | `UInt64?` | `null` | Per-request timeout in seconds. |
| `downloadDocuments` | `Bool` | `true` | Download non-HTML documents discovered during crawl. |
| `documentMaxSizeBytes` | `UInt64?` | `null` | Maximum size for downloaded documents. |
| `allowedFileTypes` | `\[String\]` | `\[\]` | MIME allowlist for downloaded documents. |
| `followDocumentUrls` | `Bool` | `false` | Follow URLs discovered inside extracted documents. |
| `documentUrlDepth` | `UInt32?` | `null` | Maximum recursive document URL depth. |
