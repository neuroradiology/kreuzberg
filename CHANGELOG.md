...
- **pdf**: AcroForm form fields are extracted into `ExtractionResult.form_fields` via `pdf_oxide`'s form API, carried through the pipeline on `InternalDocument` (no metadata round-trip). Controlled by `PdfConfig.extract_form_fields`.
